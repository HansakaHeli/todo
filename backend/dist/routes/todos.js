...
router.get("/", async (req, res) => {
    ...
    const canViewOwn = can(auth, "view_own").allowed ||
...
router.post("/", async (req, res) => {
    ...
    if (!decision.allowed)
        return res.status(403).json({ error: "Forbidden", message: decision.reason });
    const input = createTodoSchema.safeParse(req.body);
    if (!input.success)
        return res.status(400).json({ error: "Bad Request", issues: input.error.issues });
