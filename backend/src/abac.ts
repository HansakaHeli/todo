...
export type TodoAction = "view_all" | "view_own" | "create_own" | "update_own" | "delete_own_draft" | "delete_any";
...
function has(ctx: AuthContext, privilegeKey: string) {
  return ctx.privileges.includes(privilegeKey);
}

export function can(ctx: AuthContext, action: TodoAction, todo?: TodoResource): Decision {
  switch (action) {
    case "view_all":
    ...
    case "update_own": {
      if (!has(ctx, PRIV.TODO_UPDATE_OWN)) return { allowed: false, reason: "Missing privilege todo:update:own" };
      if (!todo) return { allowed: false, reason: "Missing todo context." };
      if (todo.ownerId !== ctx.user.id) return { allowed: false, reason: "Can only update your own todos." };
      return { allowed: true };
    }
    case "delete_any":
    ...
    case "delete_own_draft": {
      ...
      if (!todo) return { allowed: false, reason: "Missing todo context." };
      if (todo.ownerId !== ctx.user.id) return { allowed: false, reason: "Can only delete your own todos." };
